# 方法：回溯方法（剪枝优化）
# 剪枝：以n=4，k=4为例子，有一些枝叶的长度不足k=4，可以剪掉
...
# 3. 确定单层递归的逻辑：从startIndex开始，逐个收集结点（可以利用剪枝进行优化）
...
class Solution(object):
    ...
    def backtracking(self,n,k,startIndex,path,result):
        if len(path)==k: # 到达叶子结点，收割结果
            result.append(path[:])
            return
        for i in range(startIndex,n-(k-len(path))+2): # 剪枝优化的地方
        # len(path)是已经选取的元素的大小，还需要选取的元素的个数为k-len(path)
        # i的至多的起始位置为n-(k-len(path))+1，因此需要range的末尾为n-(k-len(path))+2
        # i模拟的就是子孩子遍历的过程
